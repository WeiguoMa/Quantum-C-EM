...
class TensorCircuit(nn.Module):
	...
	def _add_gate(self, _qubits: list[tn.Node] or list[tn.AbstractNode],
		...
		max_qubit_index = max(_oqs)
		if max_qubit_index >= self.qnumber:
			raise ValueError('Qubit index out of range.')

		single = gate.single

		if single is False:  # Two-qubit gate
			if len(_oqs) != 2 or _oqs[0] == _oqs[1]:
			...
			_axis_names = [f'physics_{_oqs[0]}', f'physics_{_oqs[1]}', f'inner_{_oqs[0]}', f'inner_{_oqs[1]}']
			if self.realNoise or self.idealNoise:
				...
				if gate.tensor.shape[-1] != 1 or len(gate.tensor.shape) != 4:
					_axis_names.append(f'I_{_oqs[1]}')

			gate = tn.Node(gate.tensor.squeeze(), name=gate.name, axis_names=_axis_names)
			...
			_edges = [tn.connect(_contract_qubits[f'physics_{_bit}'], gate[f'inner_{_bit}']) for _bit in _oqs]
			...
			if self.realNoise or self.idealNoise:
				_dup_item, _dup_idx = find_duplicate(gate.axis_names)
				if _dup_item:
					# Calculate new axis names and reshape tensor
					_length, _reshape_shape = len(gate.axis_names), list(gate.tensor.shape)
					...
					_random_string_reorder = move_index(_random_string, _dup_idx[0], _dup_idx[1] - 1)

					# Reshape the tensor
					_reshaped_tensor = tc.reshape(tc.einsum(''.join([_random_string, ' -> ', _random_string_reorder]),
					                             gate.tensor), _reshape_shape)
					_axis_names = deepcopy(gate.axis_names)
					...
					_left_qubit, _reforming_qubit, _right_qubit = None, None, None
			...
			EdgeName2AxisName([_qubits[_oqs[0]], _qubits[_oqs[1]]])

			if self.realNoise and f'I_{_oqs[1]}' in _qubits[_oqs[1]].axis_names:
				# Shape-relating
			...
			gate_list = [
				tn.Node(
					tc.reshape(
						...
						(2, 2, -1))
					if self.idealNoise or self.unified
					else gate.tensor,
					name=gate.name,
	...
	def _add_noise(self, _qubits: list[tn.Node] or list[tn.AbstractNode],
		...
		for _oq in oqs:
			...
			_noise_nodeList.append(deepcopy(_noise_node))
		...
		for _ii, _qnum in enumerate(oqs):
			...
			_dup_item, _dup_idx = find_duplicate(_qubits[_qnum].axis_names)
			if _dup_item:
				...
				_random_string = generate_random_string_without_duplicate(_length)
				_random_string_reorder = move_index(_random_string, _dup_idx[0],
				...
				_axis_names = deepcopy(_qubits[_qnum].axis_names)
				_axis_names.pop(_dup_idx[0])
	...
	def add_gate(self, gate: AbstractGate, oqs: list):
		...
		for _num, _gate in enumerate(_transpile_gateList):
			if _gate.para is None:
				...
				_paraI, _paraD = '{:.3f}'.format(tc.tensor(_gate.para).squeeze()).split('.')
	...
	def _calculate_DM(self, state_vector: bool = False,
		...
		if state_vector is False:
			_qubits_conj = deepcopy(self.state)
